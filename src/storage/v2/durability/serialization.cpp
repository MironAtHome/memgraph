...
void Encoder::WritePoint2d(storage::Point2d value) {
  ...
  WriteUint(static_cast<uint64_t>(value.crs()));
  WriteDouble(value.x());
  WriteDouble(value.y());
}
...
std::optional<Point2d> Decoder::ReadPoint2dValue() {
  ...
  return Point2d{static_cast<CoordinateReferenceSystem>(*crs), *x, *y};
}
...
std::optional<Point3d> Decoder::ReadPoint3dValue() {
  ...
  return Point3d{static_cast<CoordinateReferenceSystem>(*crs), *x, *y, *z};
}
